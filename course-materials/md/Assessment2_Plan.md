...
*   **1:15–2:00 — Docker:** In a terminal, run `wsl -d Ubuntu -u root -- bash -c "cd /mnt/c/repos/cloud-web-app/app && docker build -t phonotrail ."` then `wsl -d Ubuntu -u root -- bash -c "docker run --rm -p 3000:3000 -v phonotrail-data:/data -e DATABASE_URL=file:/data/phonotrail.db --name phonotrail-app phonotrail"`. Wait for the "Applying database migrations..." and "Starting PhonoTrail Studio..." log lines, then say "the container is now running the production build with a persisted volume."
